...
def _row_price_jpy(price_td) -> float | None:
    ...
    strike = price_td.css_first(".product-list__item__table--price-original")
    if strike is not None:
        strike.decompose()
    m = _PRICE_RE.search(price_td.text(deep=True, separator=" ", strip=True))
    return float(m.group(1).replace(",", "")) if m else None


def _row_stock(count_td) -> int:
    """0 if the row's count cell shows the "none" placeholder."""
    if count_td.css_first(".product-list__item__table--none") is not None:
        return 0
    m = _STOCK_RE.search(count_td.text(deep=True, separator=" ", strip=True))
...
def parse_search_html(html: str | bytes, card_name: str, fx_jpy_per_usd: float) -> list[dict]:
    ...
    for item in tree.css("div.product-list__item"):
        title_el = item.css_first("a.product-list__item__title--name")
        if title_el is None:
            continue
        ...
        link = (title_el.attributes.get("href") or "").strip()
        if link and not link.startswith("http"):
            link = f"{BASE_URL}{link}"

        # Each card has a price table; one row per condition grade.
        for row in item.css("table.product-list__item__table tr"):
            type_th = row.css_first("th.product-list__item__table--type")
            price_td = row.css_first("td.product-list__item__table--price")
            count_td = row.css_first("td.product-list__item__table--count")
            if not (type_th and price_td and count_td):
                continue

            condition = type_th.text(deep=True, strip=True)
